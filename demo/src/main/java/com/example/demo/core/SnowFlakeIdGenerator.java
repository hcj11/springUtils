...
import com.example.demo.core.IdGenerator;

/**
 * 1 0---0000000000 0000000000 0000000000 0000000000 0 --- 00000 ---00000
 * ---000000000000
 * <p/>
 * 在上面的字符串中，第一位为未使用（实际上也可作为long的符号位），接下来的41位为毫秒级时间，然后5位datacenter标识位，5位机器ID（
 * 并不算标识符，实际是为线程标识），然后12位该毫秒内的当前毫秒内的计数，加起来刚好64位，为一个Long型。
 * <p/>
...
public class SnowFlakeIdGenerator implements IdGenerator {
     ...
     * SEQUENCE_MASK
     */
    public static final long SEQUENCE_MASK = -1L ^ (-1L << sequenceBits);
    ...
    public SnowFlakeIdGenerator(final long workerId) {
        ...
        if ((workerId > MAX_WORKER_ID) || (workerId < 0)) {
            throw new IllegalArgumentException(String.format("算子的id不能大于 %d 或者小于 0", MAX_WORKER_ID));
        }
    ...
    public synchronized long nextId() {
        ...
        if (timestamp < this.lastTimestamp) {
            try {
                throw new Exception(String.format("系统时钟被向前调整，请校准时钟！系统拒绝生成   %d 毫秒后的id",
